...
class Model {
    ...
    void copy(Model model) {
        ...
        for (Place boundary : model.boundaries) {
            if (!boundaries.contains(boundary)) {
                boundaries.add(boundary);
            }
        }
        for (Place center : model.centers) {
            centers.add(center);
        }
    }
    ...
    void init(int cols, int rows) {
        centers = new ArrayList<Place>();
        boundaries = new ArrayList<Place>();
        xlim = 2 * cols + 1;
        ylim = 2 * rows + 1;
        table = new int[2 * cols + 1][2 * rows + 1];
        for (int xx = 0; xx < 2 * cols + 1; xx++) {
            for (int yy = 0; yy < 2 * rows + 1; yy++) {
                if (xx == 0 || xx == 2 * cols || yy == 0 || yy == 2 * rows) {
                    boundaries.add(Place.pl(xx, yy));
                }
            }
        }
    }
    ...
    boolean isCenter(int x, int y) {
        return centers.contains(Place.pl(x, y));
    }
    ...
    boolean isBoundary(int x, int y) {
        return boundaries.contains(Place.pl(x, y));
    }
    ...
    private void accreteRegion(Place cell, HashSet<Place> region) {
        ...
        for (int i = 0; i < 4; i += 1) {
            int dx = (i % 2) * (2 * (i / 2) - 1),
                dy = ((i + 1) % 2) * (2 * (i / 2) - 1);
            if (isEdge(cell.move(dx, dy).x, cell.move(dx, dy).y)
                    && !isBoundary(cell.move(dx, dy).x, cell.move(dx, dy).y)) {
    ...
    private boolean isGalaxy(Place center, HashSet<Place> region) {
        for (Place cell : region) {
            if ((isCenter(cell) && !cell.equals(center)) || !isCell(cell)) {
                return false;
            }
            for (int i = 0; i < 4; i += 1) {
                int dx = (i % 2) * (2 * (i / 2) - 1);
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                Place intersection = cell.move(dx, dy),
                        nextCell = intersection.move(dx, dy),
                        opposing = opposing(center, nextCell);
                if ((region.contains(nextCell) && !region.contains(opposing))
                        || (region.contains(intersection.move(dx, dy))
                        && isBoundary(intersection))) {
                    return false;
                }
            }
        }
    ...
    Set<Place> maxUnmarkedRegion(Place center) {
        ...
        if (unmarkedContaining(center) != null) {
            region.addAll(unmarkedContaining(center));
            ArrayList<Place> places = new ArrayList<>();
            places.addAll(region);
            markAll(region, 1);
            while (!places.isEmpty()) {
                Place place = places.remove(0);
                for (int i = 0; i < 4; i += 1) {
                    int dx = (i % 2) * (2 * (i / 2) - 1),
                            dy = ((i + 1) % 2) * (2 * (i / 2) - 1);
                    Place newPlace = place.move(2 * dx, 2 * dy);
                    Place opposingNewPlace = opposing(center, newPlace);
                    if (newPlace != null && opposingNewPlace != null
                            && mark(newPlace) == 0
                            && mark(opposingNewPlace) == 0) {
                        region.add(opposingNewPlace);
                        region.add(newPlace);
                        places.add(newPlace);
                        places.add(opposingNewPlace);
                        mark(place, 1);
                        mark(newPlace, 1);
                    }
                }
    ...
    void toggleBoundary(int x, int y) {
        if (isCell(x, y) || isIntersection(x, y)) {
            return;
        } else {
            if (!boundaries.contains(Place.pl(x, y))) {
                boundaries.add(Place.pl(x, y));
            } else {
                boundaries.remove(Place.pl(x, y));
            }
        }
    }
    ...
    void mark(int x, int y, int v) {
        if (!isCell(x, y) && !isEdge(x, y) && !isIntersection(x, y)) {
    ...
    Place opposing(Place p0, Place p) {
        if ((isCell(p0) || isIntersection(p0)
                || isEdge(p0)) && isCell(p)) {
            int dx = p.x - p0.x;
            int dy = p.y - p0.y;
            if (p0.x - dx > ylim() || p0.x - dx < 0
                    || p0.y - dy > ylim() || p0.y - dy < 0) {
                return null;
            }
            Place opposite = p0.move(-dx, -dy);
            if (isCell(opposite)) {
                return opposite;
            }
        }
    ...
    List<Place> unmarkedSymAdjacent(Place center, List<Place> region) {
        ...
        for (Place r : region) {
            ...
            for (int i = 0; i < 4; i += 1) {
                ...
                if (p != null && !isCenter(p) && mark(p) == 0
                        && opp != null && !isCenter(opp) && mark(opp) == 0) {
                    result.add(p);
                    result.add(opp);
                }
            }
